...
NSString *const imageCollectionViewElementKindSectionHeader = @"imageViewCollectionElementKindSectionHeader";
NSString *const imageCollectionViewElementKindSectionFooter = @"imageViewCollectionElementKindSectionFooter";

@interface imageCollectionViewLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id <imageCollectionViewDelegateLayout> delegate;
...
- (void)setMinimumColumnSpacing:(CGFloat)minimumColumnSpacing {
    if (minimumColumnSpacing != minimumColumnSpacing) {
        minimumColumnSpacing = minimumColumnSpacing;
        [self invalidateLayout];
    }
}

- (void)setMinimumInteritemSpacing:(CGFloat)minimumInteritemSpacing {
    if (minimumInteritemSpacing != minimumInteritemSpacing) {
...
- (void)setHeaderInset:(UIEdgeInsets)headerInset {
    if (!UIEdgeInsetsEqualToEdgeInsets(headerInset, headerInset)) {
...
- (void)setFooterInset:(UIEdgeInsets)footerInset {
    if (!UIEdgeInsetsEqualToEdgeInsets(footerInset, footerInset)) {
...
- (void)setItemRenderDirection:(imageCollectionViewLayoutItemRenderDirection)itemRenderDirection {
...
- (id <imageCollectionViewDelegateLayout> )delegate {
    return (id <imageCollectionViewDelegateLayout> )self.collectionView.delegate;
}
...
- (void)commonInit {
    ...
    _itemRenderDirection = imageCollectionViewLayoutItemRenderDirectionShortestFirst;
}
...
- (void)prepareLayout {
    ...
    NSAssert([self.delegate conformsToProtocol:@protocol(imageCollectionViewDelegateLayout)], @"UICollectionView's delegate should conform to imageCollectionViewDelegateLayout protocol");
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        if (headerHeight > 0) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:imageCollectionViewElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
        ...
        if (footerHeight > 0) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:imageCollectionViewElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section {
    ...
    switch (self.itemRenderDirection) {
        case imageCollectionViewLayoutItemRenderDirectionShortestFirst:
            index = [self shortestColumnIndexInSection:section];
            break;
            
        case imageCollectionViewLayoutItemRenderDirectionLeftToRight:
            index = (item % columnCount);
            break;
            
        case imageCollectionViewLayoutItemRenderDirectionRightToLeft:
