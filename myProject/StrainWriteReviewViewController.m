...
- (void) updateFirebaseVaslues:review_key {
    ...
    FIRDatabaseReference *userRef = [firebaseRef.usersRef child:user.user_key];

    NSLog(@"3 entered update firebase is %@ ",user.user_key);
    NSLog(@"4 entered update firebase is %@ ",strainRef);
    NSLog(@"5 entered update firebase is %@ ",userRef);
    ...
    [[strainRef child:@"rating_count" ] observeSingleEventOfType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
        ...
        [[strainRef child:@"rating_count"] setValue:string];
    }];
    
    [[[strainRef child:@"reviews"] child:review_key] setValue:@"review_key"];

    [[[userRef child:@"reviews"] child:review_key] setValue:@"review_key"];
    [[[userRef child:@"strains_tried"] child:strain.strain_key] setValue:strain.strain_name];
}
