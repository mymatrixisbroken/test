//
//  otherUserBookmarksViewController.m
//  myProject
//
...
@interface otherUserBookmarksViewController ()

@end

@implementation otherUserBookmarksViewController
...
-(void) getBookmarkInformation{
    for(int i = 0; i < [_otherUser.storeBookmarks count]; i++){
...
-(void) getBookmarkName:(NSInteger) i{
    ...
    NSString *stringKey = [_otherUser.storeBookmarks objectAtIndex:i];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    ...
    if ([_otherUser.storeBookmarks count] > 0){
        sectionCount++;
    }
    if ([_otherUser.strainBookmarks count] > 0){
        sectionCount++;
    }
    if (([_otherUser.strainBookmarks count] == 0) && ([_otherUser.storeBookmarks count] == 0)) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if(section == 0){
        return [_otherUser.storeBookmarks count];
    }
    else if(section ==1 ){
        return [_otherUser.strainBookmarks count];
    }
    return 0;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.section == 0){
        ...
        NSString *bookmarkKey = [_otherUser.storeBookmarks objectAtIndex:indexPath.row];
